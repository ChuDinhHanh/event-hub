...
const VerificationScreen = ({ navigation, route }: any) => {
    ...
    // Constant
    const ERROR_TIMING = 'Time out verification code, please resend new code to verification again!';
    ...
    const [verification, setVerification] = useState<Verification[]>(initialValue);
    const [_code, _setCode] = useState(code);
    // Process
    const handleEntryCodeEvent = useCallback((id: number, val: string) => {
        let validate = false;
    ...
    return (
        <ContainerComponent
            ...
            <SessionComponent padding={29} paddingTop={0}>
                ...
                {
                    error.isError && <TextComponent text={error.name} color={appColors.red} />
                }
                ...
                <RowComponent justifyContent='center' alignItems='center'>
                    {
                        counter > 0 ? <>
                            <TextComponent text='Re-send code in' color={appColors.black} fontFamily={fontFamilies.regular} />
                            <TextComponent text={' ' + `0:${counter}`} color={appColors.primary} fontFamily={fontFamilies.regular} />
                        </>
                            :
                            <ButtonComponent
